...
(`onboard.sh` in this repo does the same thing over the raw management API - use it if your client isn't in that list or you want to see the calls.)
...
**First win.** Ask your client:
   ...
   approved, revocable without touching the agent. And the answer is *yours* - the person next to you asking the same question gets their commits, not
...
The full loop - email trigger → Linear ticket → governed sandbox → draft PR - needs your own accounts on seven services and about an afternoon. The README's
